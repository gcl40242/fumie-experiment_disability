(function () {
  ...
  var blocks = [
    ...
    { number: 2, words: cloneWordList(block2Words), target: "○", condition: "incongruent", iatType: "不一致本番_" + TARGET_WORD },
    { number: 3, words: cloneWordList(block3Words), target: "×", condition: "congruent", iatType: "一致本番_" + TARGET_WORD },
    { number: 4, words: cloneWordList(block3Words), target: "○", condition: "incongruent", iatType: "不一致本番_" + TARGET_WORD },
    { number: 5, words: cloneWordList(block2Words), target: "×", condition: "congruent", iatType: "一致本番_" + TARGET_WORD }
  ];
  ...
  function makeInstruction(block) {
    ...
    return {
      ...
      on_load: installStartControls,
      data: { record_type: "instruction", block_number: block.number, condition: block.condition, target: TARGET_WORD, counterbalance_order: COUNTERBALANCE_ORDER }
    };
  }
  ...
  function makeTask(block) {
    return {
      ...
      data: { trial_role: "block", target: TARGET_WORD, condition: block.condition, iat_type: block.iatType, counterbalance_order: COUNTERBALANCE_ORDER },
      on_load: function () {
